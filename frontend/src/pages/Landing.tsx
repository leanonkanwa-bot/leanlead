...
const Chip = ({ children }: { children: React.ReactNode }) => (
  <span className="inline-flex items-center gap-1.5 text-xs bg-sky-950 border border-sky-800/60 text-sky-400 px-3 py-1 rounded-full font-medium">
    {children}
  </span>
);
...
function KanbanPreview() {
  const cols = [
    ...
    { label: "CONTACTED", color: "border-sky-700", count: 28 },
    { label: "REPLIED", color: "border-violet-700", count: 11 },
  ...
  return (
    <div className="relative mx-auto max-w-5xl mt-14 select-none">
      <div className="absolute inset-x-0 top-0 h-40 bg-sky-500/5 blur-3xl rounded-full pointer-events-none" />
      <div className="relative rounded-2xl border border-slate-800 bg-slate-900/80 overflow-hidden shadow-2xl backdrop-blur-sm">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-800/50">
          <span className="font-bold text-sm text-sky-400">LeanLead</span>
          <div className="flex gap-5 text-xs">
            <span className="text-sky-400 border-b border-sky-500 pb-0.5">Pipeline</span>
          ...
          <div className="text-xs bg-sky-500 text-white px-3 py-1 rounded-lg">+ Add lead</div>
        </div>
        {/* Stats */}
...
const Feature = ({ icon, title, desc }: { icon: string; title: string; desc: string }) => (
  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 hover:border-sky-900/60 transition-colors">
    <div className="w-10 h-10 bg-sky-950 rounded-xl flex items-center justify-center text-xl mb-4">{icon}</div>
...
const Pricing = ({
  ...
  <div className={`relative rounded-2xl p-8 border flex flex-col ${hot ? "bg-sky-600 border-sky-500 shadow-2xl shadow-sky-900/40" : "bg-slate-900 border-slate-800"}`}>
    ...
    <p className={`text-xs font-semibold mb-1 ${hot ? "text-sky-100" : "text-slate-400"}`}>{name}</p>
    <div className="flex items-baseline gap-1 mb-1">
      <span className="text-4xl font-black text-white">{price}</span>
      {price !== "Free" && <span className={`text-sm ${hot ? "text-sky-200" : "text-slate-500"}`}>/mo</span>}
    </div>
    {sub && <p className={`text-xs mb-6 ${hot ? "text-sky-100" : "text-slate-500"}`}>{sub}</p>}
    <ul className="space-y-3 flex-1 mb-8">
      {features.map(f => (
        <li key={f} className="flex gap-2 text-sm">
          <span className={hot ? "text-sky-100" : "text-emerald-400"}>✓</span>
          <span className={hot ? "text-sky-50" : "text-slate-300"}>{f}</span>
        </li>
      ))}
    </ul>
    <Link to="/register" className={`text-center py-3 rounded-xl text-sm font-semibold transition-colors ${hot ? "bg-white text-sky-700 hover:bg-sky-50" : "bg-slate-800 text-white hover:bg-slate-700 border border-slate-700"}`}>
      {cta}
    </Link>
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 border-b border-slate-900 bg-slate-950/90 backdrop-blur-md">
        <span className="font-extrabold text-lg">Lean<span className="text-sky-400">Lead</span></span>
        ...
        <div className="flex gap-3 items-center">
          ...
          <Link to="/register" className="text-sm bg-sky-500 hover:bg-sky-400 px-4 py-2 rounded-lg font-medium transition-colors shadow-lg shadow-sky-900/30">Start free</Link>
        </div>
      </nav>

      {/* Hero */}
      <section className="relative px-6 pt-20 pb-8 text-center overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[300px] bg-sky-600/8 blur-3xl rounded-full pointer-events-none" />
        ...
        <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/register" className="px-8 py-3.5 bg-sky-500 hover:bg-sky-400 rounded-xl font-semibold text-sm transition-colors shadow-lg shadow-sky-900/40">
            Start for free →
          </Link>
          ...
          <div className="space-y-0">
            {[
              ...
              <div key={s.n} className="flex gap-6 pb-12 relative">
                ...
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-sky-950 border border-sky-800 flex items-center justify-center z-10">
              ...
              <div key={t.name} className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                ...
                <div className="flex justify-between items-end">
                  ...
                  <div className="text-right">
                    <p className="text-sky-400 font-bold text-sm">{t.result}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-24 px-6 text-center">
        ...
        <Link to="/register" className="inline-block px-10 py-4 bg-sky-500 hover:bg-sky-400 rounded-xl font-bold text-base transition-colors shadow-xl shadow-sky-900/40">
      ...
      <footer className="border-t border-slate-900 py-8 px-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-600">
        <span className="font-bold text-slate-500">Lean<span className="text-sky-600">Lead</span></span>
